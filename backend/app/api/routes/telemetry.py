...
def history(device_id: str, channel: str | None = None, user: User = Depends(current_user), db: Session = Depends(get_db)):
    stmt = select(Telemetry).where(Telemetry.tenant_id == user.tenant_id, Telemetry.device_id == device_id)
    if channel:
        stmt = stmt.where(Telemetry.channel == channel)
    return [_response(row) for row in db.scalars(stmt.order_by(desc(Telemetry.observed_at)).limit(1000)).all()]
